...
export default function RandomHanyu({ words }) {
      ...
      <Card sx={{ width: '100%', marginTop: 1 }}>
        <CardContent>
          <Box sx={{ display: 'flex', justifyItems: 'center', justifyContent: 'space-between' }}>
            <Typography fontWeight="bold">Pinyin - Nhập chữ hán</Typography>
          ...
          <Box mx="auto" mt={2}>
            <Stack spacing={2}>
              {arrData?.map(word => (
                ...
                >
                  <Typography sx={{ width: '50%', fontSize: 20 }}>{word?.pinyin}</Typography>
                  <TextField
                    label="Chữ hán"
                    variant="standard"
                    size="small"
